...
const oneWindow = document.querySelector("#window1");   // Получаем достум к первому окну с результатами
const twoWindow = document.querySelector("#window2");   // Получаем доступ к второму окну с алфавитом\буквами
const threeWindow = document.querySelector("#window3");  // Получаем доступ к третьему окну старт\клавиатура
const footerWindow = document.querySelector("#window4"); // Получаем доступ к Футеру
...
function updateWindowTwo() {   //Функция заполнения второго окна
    twoWindow.innerHTML = `<button id="help"><p>Подсказка</p></button><img id="imgAlph" class="imgAlp" src=${srcPic}>`;
...
function getAlphabet() {    // Функция выводит увеличенный алфавит
    ...
    btnAlphabet.onclick = () => {       // Когда кнопка нажимается запускается функция
        ...
        img.classList.remove("imgAlp");           // удаляем старые стили
        img.classList.add("fotoAlphabet");      // Устанавливаем новые стили с позицией абсолют и большим размером фото
        img.onclick = () => {        // Ждем клика на картинку и запускаем функцию
            img.classList.remove("fotoAlphabet");   //  Удаляем стили
            img.classList.add("imgAlp");   //  Возвращаем старые стили
        }
    };
}
...
function start() {   // Главная функция\ весь алгоритм работы тренажера здесь
    ...
    btnStop.onclick = function() {       // Ожидаем нажатия кнопки Stop и после нажатия выполняем функцию
        ...
        getAlphabet() // При нажатии кнопки "подсказка" увеличивает алфавит на весь экран
    }
